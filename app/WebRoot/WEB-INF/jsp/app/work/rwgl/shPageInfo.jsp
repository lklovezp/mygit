...
<div class="checkup">
    ...
    <table class="dataTable" width="100%" border="1px" bordercolor="#d4d4d4"  cellpadding="0" cellspacing="0">
        ...
        <tr>
	        <td colspan="8">
	           <a href="#" id="viewDetail" style="float: right;color: #0088CC;" onclick="info()">任务详情</a>&nbsp;
	        </td>
        </tr>
        <tr  height="42">
...
$(document).ready(function() {
	...
	$('#xbryTable').datagrid({
	    ...
	    columns:[[
	        ...
	        {field:'workmobile',title:'电话',width:120,align:'center'},
	        {field:'lawnumber',title:'执法证号',width:120,align:'center'},
	        {field:'org',title:'部门',width:120,align:'center'}
	    ]]
	});
	...
	$('#zfdxTable').datagrid({
		...
		fitColumns:true,
	    columns:[[  
	        {field:'lawobjname',title:'执法对象',width:110,align:'center'},   
	  ...
	  success:function(data){
	      if(data.rwlxIds!=null&&data.rwlxIds!=''){
	          ...
	          $.each(codesArr, function(i, n){
		            ...
		            case '10'://现场监察
		               rwlxObj=new RwlxObj("10","现场监察","RCJC","RCJCJCJL,RCJCMOREJCBL,RCJCJCJLSMJ,RCJCJCBG,RCJCXZWS,RCJCQTZL,RCJCCLYJS,RCJCZBZL,RCJCSPZL,RCJCLYZL,RCJCZP,RCJCHPPFWJ,RCJCYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		               ...
		               break;
		            case '11'://年度核查
		               rwlxObj=new RwlxObj("11","年度核查","NDHC","NDHCJCJL,NDHCMOREJCBL,NDHCJCJLSMJ,NDHCJCBG,NDHCXZWS,NDHCQTZL,NDHCCLYJS,NDHCZBZL,NDHCSPZL,NDHCLYZL,NDHCZP,NDHCHPPFWJ,NDHCYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		               ...
		               break;
		            case '12'://后督察
		               rwlxObj=new RwlxObj("12","后督察","HDC","HDCJCJL,HDCMOREJCBL,HDCJCJLSMJ,HDCJCBG,HDCXZWS,HDCQTZL,HDCCLYJS,HDCZBZL,HDCSPZL,HDCLYZL,HDCZP,HDCHPPFWJ,HDCYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		            ...
		            case '13'://信访投诉
		               ...
		               break;
		            case '14'://排污许可证检查
		               rwlxObj=new RwlxObj("14","排污许可证检查","PWXKZJC","PWXKZJCJCJL,PWXKZJCMOREJCBL,PWXKZJCJCJLSMJ,PWXKZJCJCBG,PWXKZJCXZWS,PWXKZJCQTZL,PWXKZJCCLYJS,PWXKZJCZBZL,PWXKZSPZL,PWXKZLYZL,PWXKZZP,PWXKZHPPFWJ,PWXKZYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		            ...
		            case '16'://违法案件调查
		               ...
		               break;
		            case '17'://限期治理
		               rwlxObj=new RwlxObj("17","限期治理","XQZL","XQZLJCJL,XQZLMOREJCBL,XQZLJCJLSMJ,XQZLJCBG,XQZLXZWS,XQZLQTZL,XQZLCLYJS,XQZLZBZL,XQZLSPZL,XQZLLYZL,XQZLZP,XQZLHPPFWJ,XQZLYSPFWJ,XQZLHJWFXWXQGZTZ,XQZLXZCFJDHSDHZ,XQZLTZGZSSDHZ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		               ...
		               break;
		            case '18'://跟踪检查
		               rwlxObj=new RwlxObj("18","跟踪检查","GZJC","GZJCJCJL,GZJCMOREJCBL,GZJCJCJLSMJ,GZJCJCBG,GZJCXZWS,GZJCQTZL,GZJCCLYJS,GZJCZBZL,GZJCSPZL,GZJCLYZL,GZJCZP,GZJCHPPFWJ,GZJCYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		            ...
		            case '19'://自动监控
		               ...
		               break;
		            case '20'://危险废物
		               rwlxObj=new RwlxObj("20","危险废物","WXFW","WXFWJCJL,WXFWMOREJCBL,WXFWJCJLSMJ,WXFWJCBG,WXFWXZWS,WXFWQTZL,WXFWCLYJS,WXFWZBZL,WXFWSPZL,WXFWLYZL,WXFWZP,WXFWHPPFWJ,WXFWYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
		            ...
		            case '21'://危险化学品
		               ...
		               break;
		            case '22'://辐射安全
		               rwlxObj=new RwlxObj("22","辐射安全","FSAQ","FSAQJCJL,FSAQMOREJCBL,FSAQJCJLSMJ,FSAQJCBG,FSAQXZWS,FSAQQTZL,FSAQCLYJS,FSAQZBZL,FSAQSPZL,FSAQLYZL,FSAQZP,FSAQHPPFWJ,FSAQYSPFWJ");
		               //根据任务类型获取监察笔录
					   $.ajax({
...
function showCOMMON(rwlxObj){
    ...
    $('#'+rwlxObj.pyjx+'data').datagrid({
		...
		columns:[[
			...
			{field:'ck',title:'是否上报',checkbox:true}
		]],
		singleSelect: false,
		selectOnCheck: true,
        checkOnSelect: false,
        onLoadSuccess:function(data){
            $('#'+rwlxObj.pyjx+'data').datagrid('checkAll');
	        if(data){
	            $.each(data.rows, function(index, item){
	                if(item.checked){
	                    $('#'+rwlxObj.pyjx+'data').datagrid('checkRow', index);
	                }
	            });
	        }
	    }
...
function zfdxInfo(obj){
	var href = 'lawobjInfo.htm?id='+obj.id; 
  	var width=screen.width * 0.8;
  	var height=screen.height * 0.8-50;
	parent.parent.layerIframe(2,href,"执法对象信息查看",width,height);
   // All.ShowModalWin('lawobjInfo.htm?id='+obj.id, '执法对象信息查看');
}
...
function info(){
   ...
   //  All.ShowModalWin('taskDetail.htm?applyId='+applyId, '任务详情');
}
...
function getDatagrid(){
    var ids = [];
    $("div[id^='DIV_']:visible table[id$='data']").each(function(i){
        var checkedItems = $(this).datagrid('getChecked');
