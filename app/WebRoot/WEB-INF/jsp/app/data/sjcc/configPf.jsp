		...
		<div class="dataDiv" style="border-right: 1px;width: 98%;padding: 0px 0px 0px 14px;">
			<form id="myform"  name="myform" method="post">
				...
				<table class="dataTable" width="100%">
					<tr>
					    <th width="100"><font color="red"> * </font>区域：</th>
						<td width="200" align="center"><input class="y-text" id="areaid" name="areaid"
							type="text" value="${areaid}" data-options="required:true" />
						</td>
						<th>
							<font color="red"> * </font>年份：
						</th>
						<td>
							<input class="y-text" type="text" id="year" name="year" data-options="required:true" vale=""/>
						</td>
						<th>
							<font color="red"> * </font>月份：
						</th>
						<td>
							<input class="y-text" type="text" id="month" name="month" data-options="required:true" value="${configPfForm.month}"/>
						</td>
						<th>
...
$(document).ready(function(){
	...
	$('#areaid').combobox({
		...
		onSelect: function (param) {//做区域部门的联动选择
			...
			$.ajax({
				url: 'configPfList.json?year='+$('#year').combobox('getValue')+'&month='+$('#month').combobox('getValue')+'&areaid='+param.id+'&type='+'1', 
				type: "post", 
	...
	$('#year').combobox({
		...
		onSelect: function(param){
			$('#functionOp').datagrid('endEdit', lastIndex);
			$.ajax({
				url: 'configPfList.json?year='+param.id+'&month='+$('#month').combobox('getValue')+'&areaid='+$('#areaid').combobox('getValue')+'&type='+'1', 
				type: "post", 
				success: function(data){
					var s=0;
					if(data.length>0){
					...
					//$('#bjsx').val();
					$('#bjsx').numberbox('setValue', s);
					//document.getElementById("bjsx").value=s;
				$('#functionOp').datagrid('loadData', data);
		      }});
	...
	$('#month').combobox({
		...
		onSelect: function(param){
			$('#functionOp').datagrid('endEdit', lastIndex);
			$.ajax({
				url: 'configPfList.json?year='+$('#year').combobox('getValue')+'&month='+param.id+'&areaid='+$('#areaid').combobox('getValue')+'&type='+'1', 
				type: "post", 
				success: function(data){
					//alert();
					var s=0;
	...
	dataG = $('#functionOp').datagrid({
		url:'configPfList.json?year='+$('#year').combobox('getValue')+'&month='+$('#month').combobox('getValue')+'&type='+'1',
		toolbar:[{
			text:'添加',
			iconCls:'icon-add',
			handler:function(){
	...
	submitHandler:function(form){
		...
		if($("#myform").form("validate")){
			  ...
			  var validData = dataG.datagrid('getData');
	           ...
	           data: "year="+$('#year').combobox('getValue')+"&month="+$('#month').combobox('getValue')+"&areaid="+$('#areaid').combobox('getValue'),
