...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>建筑工地信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="05"/>
			<div id="condition">
				${innerHtml }
			</div>
...
$(document).ready(function(){
	...
	$("#J-from-reset").click(function(){
		   ...
		   var $selectedvalue = $("input[id='jzgd_qysczt']:checked").val();
		   if ($selectedvalue == 'Y') {
				$("#season").hide();
				$("#jzgd_qysczt").val("Y");
		   }
...
$(document).ready(function() {
	...
	$("input[id='jzgd_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='jzgd_qysczt']").each(function(){
...
function setValues(id,dwmc,dwdz,fddbr,fddbrlxdh,hbfzr,hbfzrlxdh){
	$("#jzgd_sgdwmc").val(dwmc);
	if(dwdz!=""){
		$("#jzgd_dz").val(dwdz);
	}
	if(fddbr!=""){
		$("#jzgd_fddbr").val(fddbr);
	}
	if(fddbrlxdh!=null){
		$("#jzgd_fddbrlxdh").val(fddbrlxdh);
	}
	if(hbfzr!=""){
		$("#jzgd_hbfzr").val(hbfzr);
	}
	if(hbfzrlxdh!=""){
		$("#jzgd_hbfzrlxdh").val(hbfzrlxdh);
	}
	jQuery().colorbox.close();
}
</script>