...
<html>
	...
	<body>
        ...
        <div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;">
			<form id="queryForm" action="queryJzgdList.json" method="post">
				...
				<table class="dataTable" width="100%"  bordercolor="#d4d4d4" cellpadding="0" cellspacing="0">
					<tr>
						...
						<td width="200" align="left">
							<input class="y-text" type="text" name="sgdwmc" id=""/>
						</td>
						<th width="120" align="right">
							所属行政区：
						</th>
...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		nowrap:false,
		fitColumns:true,
		url:'queryJzgdList.json?isActive=Y&qyzt=0&orgId='+$('#ssjgbm').val(),
		...
		columns:[[
			...
			{field:'sgdwmc',title:'施工单位名称', align:"left", halign:'center',width:100},
		...
		onDblClickRow: function(rowIndex, rowData){
			//All.ShowModalWin('jzgdInfo.htm?id='+rowData.id, '查看建筑工地信息');
			var width=screen.width * 0.8;
		  	var height=screen.height * 0.8-50;
		  	var title='查看建筑工地信息';
		  	parent.layerIframe(2,'jzgdInfo.htm?id='+rowData.id,title,width,height);
		}
	});
	initPager();
});
...
function edit(obj){
	...
	parent.layer.open({
        ...
        title: '新建建筑工地',
        area: ['1100px', ($(window).height()+120)+'px'],
        content: 'jzgdEdit.htm?id='+$(obj).attr("id"), 
        end: function () {
        	refresh();
        }
...
$("#daoru").click(function(){
	//All.ShowModalWin('importPage.htm?lawObjType=05', '工业污染源导入', 600, 350);
	//refresh();
	layer.open({
        type: 2,
        title: '建筑工地导入',
        area: ['600px', ($(window).height()-50)+'px'],
        content: 'importPage.htm?lawObjType=05', 
        end: function () {
        	refresh();
        }
...
function info(obj){
	//All.ShowModalWin('jzgdInfo.htm?id='+$(obj).attr("id"), '查看建筑工地信息');
	var width=screen.width * 0.8;
  	var height=screen.height * 0.8-50;
  	var title='查看建设项目信息';
  	parent.layerIframe(2,'jzgdInfo.htm?id='+$(obj).attr("id"),title,width,height);
}

//创建人转移
function transfer(obj){
	//All.ShowModalWin('lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=05&multi=false&lawobjId='+$(obj).attr("id"), '创建人转移',300, 380);
	parent.layer.open({
        type: 2,
        title: '创建人转移',
        area: ['300px', '420px'],
        content: 'lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=05&multi=false&lawobjId='+$(obj).attr("id"),
        end: function () {
        	refresh();
        }
