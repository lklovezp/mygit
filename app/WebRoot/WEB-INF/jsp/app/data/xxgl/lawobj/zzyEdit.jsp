...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>制造业信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="10"/>
...
$(document).ready(function(){
	...
	$("#zzy_wghzrr").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	$("#fjlx").combobox({
	...
	$("#zzy_dwmc").val('${jsxmForm.dwmc }');
	$("#zzy_dz").val('${jsxmForm.address }');
	$("#zzy_fddbr").val('${jsxmForm.fddbr }');
	$("#zzy_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
	$("#zzy_hbfzr").val('${jsxmForm.hbfzr }');
	$("#zzy_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
	$("#zzy_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	
	$("#J-from-reset").click(function(){
		   $("#queryForm").form("reset");
		   var $selectedvalue = $("input[id='zzy_qysczt']:checked").val();
			if ($selectedvalue == 'Y') {
				 $("#season").hide();
				 $("#zzy_qysczt").val("Y");
			}
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存制造业信息:", data.msg, "info", function () {
							    //self.close();
							...
							$.messager.alert('保存制造业信息:',data.msg);
						}
					}
				});
			}
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前制造业信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前制造业信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
...
function setUserValue(id,name){
	//$("#zzy_wghzrr").linkbox("setValue", {id:id,name:name});
	$("#zzy_wghzrr").val(name);
	$("#zzy_wghzrrid").val(id);
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='zzy_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='zzy_qysczt']").each(function(){
