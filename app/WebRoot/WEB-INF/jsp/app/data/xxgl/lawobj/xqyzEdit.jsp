...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>畜禽养殖信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="07"/>
...
$(document).ready(function(){
	...
	$("#xqyz_wghzrr").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	...
	$("#xqyz_dwmc").val('${jsxmForm.dwmc }');
	$("#xqyz_dz").val('${jsxmForm.address }');
	$("#xqyz_fddbr").val('${jsxmForm.fddbr }');
	$("#xqyz_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
	$("#xqyz_hbfzr").val('${jsxmForm.hbfzr }');
	$("#xqyz_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
	$("#xqyz_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	$("#J-from-reset").click(function(){
		   $("#queryForm").form("reset");
		   var $selectedvalue = $("input[id='xqyz_qysczt']:checked").val();
		   if ($selectedvalue == 'Y') {
				$("#season").hide();
				$("#xqyz_qysczt").val("Y");
		   }
	...
	$('#data').datagrid({
		...
		pagination:true,
		fitColumns:true,
		url:'queryFileList.json?pid='+$("input[name='id']").val(),
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存畜禽养殖:", data.msg, "info", function () {
							    //self.close();
							...
							$.messager.alert('保存畜禽养殖:',data.msg);
						}
					}
				});
			}
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前畜禽养殖信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前畜禽养殖信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
							...
							success: function(data){
								if(data.state){
									...
									$.messager.alert('保存畜禽养殖信息:',data.msg);
...
function setUserValue(id,name){
	//$("#xqyz_wghzrr").linkbox("setValue", {id:id,name:name});
	$("#xqyz_wghzrr").val(name);
	$("#xqyz_wghzrrid").val(id);
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='xqyz_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='xqyz_qysczt']").each(function(){
