...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>饮食业信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="09"/>
...
$(document).ready(function(){
	...
	$("#ysy_wghzrr").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	$("#fjlx").combobox({
	...
	$("#ysy_dwmc").val('${jsxmForm.dwmc }');
	$("#ysy_dz").val('${jsxmForm.address }');
	$("#ysy_fddbr").val('${jsxmForm.fddbr }');
	$("#ysy_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
	$("#ysy_hbfzr").val('${jsxmForm.hbfzr }');
	$("#ysy_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
	$("#ysy_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	
	$("#J-from-reset").click(function(){
		   $("#queryForm").form("reset");
		   var $selectedvalue = $("input[id='ysy_qysczt']:checked").val();
			if ($selectedvalue == 'Y') {
				 $("#season").hide();
				 $("#ysy_qysczt").val("Y");
			}
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存饮食业信息:", data.msg, "info", function () {
							   //self.close();
							...
							$.messager.alert('保存饮食业信息:',data.msg);
						}
					}
				});
			}
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前饮食业信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前饮食业信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
...
function setUserValue(id,name){
	//$("#ysy_wghzrr").linkbox("setValue", {id:id,name:name});
	$("#ysy_wghzrr").val(name);
	$("#ysy_wghzrrid").val(id);
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='ysy_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='ysy_qysczt']").each(function(){
