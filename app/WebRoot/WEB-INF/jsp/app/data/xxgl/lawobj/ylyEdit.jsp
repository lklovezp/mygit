...
<html>
	...
	<body style="padding-bottom:70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>娱乐业信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="11"/>
...
$(document).ready(function(){
	...
	$("#yly_wghzrr-link").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	$("#fjlx").combobox({
	...
	$("#yly_dwmc").val('${jsxmForm.dwmc }');
	$("#yly_dz").val('${jsxmForm.address }');
	$("#yly_fddbr").val('${jsxmForm.fddbr }');
	$("#yly_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
	$("#yly_hbfzr").val('${jsxmForm.hbfzr }');
	$("#yly_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
	$("#yly_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	
	$("#J-from-reset").click(function(){
		   $("#queryForm").form("reset");
		   var $selectedvalue = $("input[id='yly_qysczt']:checked").val();
			if ($selectedvalue == 'Y') {
				 $("#season").hide();
				 $("#yly_qysczt").val("Y");
			}
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存娱乐业信息:", data.msg, "info", function () {
							    //self.close();
							...
							$.messager.alert('保存娱乐业信息:',data.msg);
						}
					}
				});
			}
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前娱乐业信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前娱乐业信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
...
function setUserValue(id,name){
	$("#yly_wghzrr").linkbox("setValue", {id:id,name:name});
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='yly_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='yly_qysczt']").each(function(){
