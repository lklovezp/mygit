...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>医院信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" id="lawobjId" name="id" value="${lawobj.id }"/>
			<input type="hidden" id="lawobjType" name="lawobjtype" value="03"/>
			<input type="hidden" name="jsxmid" id="jsxmid" value="${jsxmForm.id }"/>
			<div id="condition">
				${innerHtml}
			</div>
...
$(document).ready(function(){
	...
	$("#yy_wghzrr").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	
	//设置建设项目转过来的属性值
	<c:if test="${jsxmForm!=null }">
		$("#yyxx_dwmc").val('${jsxmForm.dwmc }');
		$("#yy_yydz").val('${jsxmForm.address }');
		$("#yyxx_fddbr").val('${jsxmForm.fddbr }');
		$("#yyxx_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
		$("#yyxx_hbfzr").val('${jsxmForm.hbfzr }');
		$("#yyxx_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
		$("#yyxx_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	...
	$("#J-from-reset").click(function(){
		   ...
		   var $selectedvalue = $("input[id='yy_qysczt']:checked").val();
		   if ($selectedvalue == 'Y') {
				$("#season").hide();
				$("#yy_qysczt").val("Y");
		   }
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存医院信息:", data.msg, "info", function () {
							    //self.close();
							...
							$.messager.alert('保存医院信息:',data.msg);
						}
					}
				});
			}
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前医院信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前医院信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
...
function setUserValue(id,name){
	//$("#yy_wghzrr").linkbox("setValue", {id:id,name:name});
	$("#yy_wghzrr").val(name);
	$("#yy_wghzrrid").val(id);
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='yy_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='yy_qysczt']").each(function(){
