...
<body>
        ...
        <div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;">
			<form id="queryForm" action="queryScxxList.json" method="post">
...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		nowrap:false,
		fitColumns:true,
		url:'queryScxxList.json?isActive=Y&qyzt=0&orgId='+$('#ssjgbm').val(),
		...
		onDblClickRow: function(rowIndex, rowData){
			//All.ShowModalWin('scxxInfo.htm?id='+rowData.id, '查看三产信息');
			var width=screen.width * 0.8;
		  	var height=screen.height * 0.8-50;
		  	var title='查看三产信息';
		  	parent.layerIframe(2,'scxxInfo.htm?id='+rowData.id,title,width,height);
		}
	});
	initPager();
});
...
function edit(obj){
	...
	parent.layer.open({
        ...
        title: '编辑三产信息',
        area: ['1100px', ($(window).height()+120)+'px'],
        content: 'scxxEdit.htm?id='+$(obj).attr("id"), 
        end: function () {
        	refresh();
        }
...
$("#daoru").click(function(){
	//All.ShowModalWin('importPage.htm?lawObjType=06', '工业污染源导入', 600, 350);
	//refresh();
	layer.open({
        type: 2,
        title: '三产信息导入',
        area: ['600px', ($(window).height()-50)+'px'],
        content: 'importPage.htm?lawObjType=06', 
        end: function () {
        	refresh();
        }
...
function info(obj){
	...
	var width=screen.width * 0.8;
  	var height=screen.height * 0.8-50;
  	var title='查看三产信息';
  	parent.layerIframe(2,'scxxInfo.htm?id='+$(obj).attr("id"),title,width,height);

}

//创建人转移
function transfer(obj){
	//All.ShowModalWin('lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=06&multi=false&lawobjId='+$(obj).attr("id"), '创建人转移',300, 380);
	parent.layer.open({
        type: 2,
        title: '创建人转移',
        area: ['300px', '420px'],
        content: 'lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=06&multi=false&lawobjId='+$(obj).attr("id"),
        end: function () {
        	refresh();
        }
