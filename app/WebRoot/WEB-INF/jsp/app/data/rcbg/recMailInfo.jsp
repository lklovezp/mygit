...
<input type="hidden" id="isHuiFu" value="${isHuiFu}"/>
<input type="hidden" id="topic" value="${mailForm.topic}"/>
<table class="formtable" width="90%"  cellpadding="0" cellspacing="0" style="margin:0 auto 20px;">
	<tr>
     <th width="90" align="right" style="background-color:#cff1ff">发送人：</th>
    ...
    <tr>
     <th width="90" align="right" style="font-size:14px;background-color:#cff1ff" valign="top" >附　件：</th>
     <td>
        <div class="fileList">
    ...
    <tr>
     <th width="90" align="right" style="font-size:14px;background-color:#cff1ff" valign="top">内　容：</th>
...
<div class="allViews" >
    ...
    <c:forEach  items ="${yjList}" var = "yj">
      <li>
        ...
        <p>${yj.yiJianContent}</p> <p>${yj.fuJianId}</p>
      </li>
    </c:forEach>
    </ul>
</div>
...
<div style="height:40px; text-align:center" class="rb_btn_fix">
   <c:if test="${isHuiFu=='N'}">
  <font color="red">请及时回复意见→</font>&nbsp;&nbsp;&nbsp;&nbsp;
   </c:if>
   <input  type="button" value="回复意见"  class="queryBlue" onclick="toHuiFuYijian()">
   <input  type="button" value="返回" class="queryOrange" onclick="fanhui()">
</div>
</form>
</body>
</html>
...
window.onunload = window.onbeforeunload = function() {
	if($('#isHuiFu').val()=='N'){
		parent.layer.open({
	        ...
	        content: 'guanBiHuiFu.htm?mailId='+$("#mailId").val()+'&recMailId='+$("#recMailId").val() 
	    });
		//All.ShowModalWin('guanBiHuiFu.htm?mailId='+$("#mailId").val()+'&recMailId='+$("#recMailId").val(), '回复意见',800,600);
		$('#isHuiFu').val("Y");
	}
...
function toHuiFuYijian(){
	...
	layer.open({
        ...
        title: '回复意见',
        area: ['800px', ($(window).height()-50)+'px'],
        content: 'huiFuYiJian.htm?mailId='+$("#mailId").val()+'&recMailId='+$("#recMailId").val(), 
        end: function () {
        	$('#isHuiFu').val("Y");
            location.reload();
        }
...
function fanhui(){
	if($('#isHuiFu').val()=='N'){
		...
		layer.open({
	        ...
	        content: 'guanBiHuiFu.htm?mailId='+$("#mailId").val()+'&recMailId='+$("#recMailId").val(), 
	        	...
	        	window.location.href="recConsultation.htm?title=已收会商&fid=40288ab75591443e01559148bf740003";
	...
	//}
	//parent.dkSelect("","首页","home.htm");
	//parent.dkSelect("","已收会商","queryRecConsultList.json");
