...
<div class="dataDiv" style="width:95%;min-width: 1020px; margin:16px auto 25px;">
    ...
    <div class="mt25" id="annex" style="width:33%; height:300px; float:left">
        <div class="annex_header" style="font-size: 16px;">
       <!--  <input type="button"  onclick="delrow();" id="del"  class="o_btn btn_blue" style="float:right; line-height: 30px; color:#fff;margin-top:7px;font-size: 14px;" value="删除"/>
    ...
    <input id="save" type="submit" class="o_btn btn_blue" value="保存">
        ...
        columns:[[
            ...
            {field:'operate',title:'操作', align:"center", halign:'center',width:3,
		    	formatter:function(value,rowData,rowIndex){  
		    		return '<a class="b-link" onclick="delrow(\''+ rowData.lawobjfid +'\')">删除</a> ';  
		       }  
			}
...
$("#J-from-reset").click(function(){
	...
	$("#save").click(function(){
		...
		var rowString = '[';
		for(var i=0;i<rows.length;i++){
			rowString += "{'lawobjid':'"+rows[i].lawobjfid+"'";
			rowString += ",'lawobjname':'"+rows[i].name+"'";
		...
		$.ajax({
			  ...
			  data: "rows="+encodeURIComponent(rowString)+"&lawobjtype="+$("#lawobjtypeid").val()+"&rwid="+$("#rwid").val(),
			  success:function(data){
				  if(data.state){
						if(bs == '1'){
							window.parent.huixian(nameString,rowString,idString);
