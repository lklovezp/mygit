...
$('#data').datagrid({
	...
	fitColumns : true,
	pagination:true,
	url:'queryLawdocList.json',//请求数据的超链接地址
	...
	if(!isParent($("#pid").val())){
		var index=parent.layer.confirm('确定删除吗？', {
	        ...
	        title:'删除任务'
	    	 ...
	    	 $("#queryForm").ajaxSubmit( {
	    		    type:"post",
					url:"deleteLawdocdir.json?id="+$("#pid").val(),
					dataType:"json",
					success : function(data) {
						if(data.state){
						    location.reload();
...
$("#functionAdd").click(function(){
	parent.layer.open({
        ...
        title: '添加执法文件目录',
        area: ['800px', '500px'],
        content: 'editLawdocdir.htm?pid='+$("#pid").val(), 
        end: function () {
            location.reload();
...
$("#functionEdit").click(function(){
	...
	parent.layer.open({
		       ...
		       shadeClose:false,
		       maxmin:true,
		       content:'editLawdocdir.htm?id='+$("#pid").val(),
		       end:function(){
		    	   location.reload();
		       }
		     });
