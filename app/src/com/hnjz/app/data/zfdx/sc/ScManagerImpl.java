 ...
 * @version 
 */
 @Service("TDataScManagerImpl")
public class ScManagerImpl extends ManagerImpl implements ScManager{


/** 日志 */
	private static final Log log = LogFactory.getLog(ScManagerImpl.class);
	 ...
	 * @throws AppException
	 */
	public TDataLawobjf savesc(ScForm frm) throws AppException{
       ...
       TDataSc po1old= new TDataSc();
		if(frm.getLawobjfid()!=null && frm.getLawobjfid()!=""){
			po1= (TDataSc) this.getsc(frm.getLawobjfid());
...
/**
	...
	public void editsc(ScForm frm){
       ...
       TDataSc po=this.getsc(frm.getId());
	...
	public TDataSc getsc(String lawobjid) {
    	...
    	TDataSc sc = null;
		String hsql = " from TDataSc where lawobjf.id=?";
		List<TDataSc> re = this.dao.find(hsql, lawobjid);
		if (re.isEmpty()) {
			return null;
		} else {
			sc = re.get(0);
	...
	public void delSc(String lawobjid) {
		// TODO Auto-generated method stub
		TDataSc del = this.getsc(lawobjid);
