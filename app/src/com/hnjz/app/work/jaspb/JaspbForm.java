package com.hnjz.app.work.jaspb;

/**
 * 行政处罚案件结案审批表的form
 * 作者：zhangqingfeng
 * 生成日期：Mar 23, 2016
 * 功能描述：
 */
public class JaspbForm {
	/** id **/
	String id;
	
	/** 任务ID */
	private String taskid;
	/** 案由 */
	private String ajnr;
	/** 案件来源 */
	private String ajly;
	/** 当事人（名称或姓名） */
	private String dsrmc;
	/** 法定代表人 */
	private String fddbr;
	/** 工作单位 */
	private String company;
	/** 职务或职业*/
	private String dsrzw;
	/** 地址或住址 */
	private String dsrdz;
	/** 立案时间 */
	private String lasj;
	/** 案件审查人 */
	private String ajscr;
	/** 执法证编号 */
	private String ajscrbh;
	/** 行政处罚决定书文号 */
	private String xzcfjdswh;
	/** 简要案情及查处经过 */
	private String jyaq;
	/** 处理依据及结果 */
	private String clyj;
	/** 行政复议行政诉讼情况 */
	private String xzssqk;
	/** 处罚执行情况及罚没财务的处置情况 */
	private String cfzxqk;
	/** 备注 */
	private String remark;
	/** 数据区域过滤标识 */
	private String areaId;
	public String getId() {
		return id;
	}
	public void setId(String id) {
		this.id = id;
	}
	public String getTaskid() {
		return taskid;
	}
	public void setTaskid(String taskid) {
		this.taskid = taskid;
	}
	public String getAjnr() {
		return ajnr;
	}
	public void setAjnr(String ajnr) {
		this.ajnr = ajnr;
	}
	public String getAjly() {
		return ajly;
	}
	public void setAjly(String ajly) {
		this.ajly = ajly;
	}
	public String getDsrmc() {
		return dsrmc;
	}
	public void setDsrmc(String dsrmc) {
		this.dsrmc = dsrmc;
	}
	public String getFddbr() {
		return fddbr;
	}
	public void setFddbr(String fddbr) {
		this.fddbr = fddbr;
	}
	public String getCompany() {
		return company;
	}
	public void setCompany(String company) {
		this.company = company;
	}
	public String getDsrzw() {
		return dsrzw;
	}
	public void setDsrzw(String dsrzw) {
		this.dsrzw = dsrzw;
	}
	public String getDsrdz() {
		return dsrdz;
	}
	public void setDsrdz(String dsrdz) {
		this.dsrdz = dsrdz;
	}
	public String getLasj() {
		return lasj;
	}
	public void setLasj(String lasj) {
		this.lasj = lasj;
	}
	public String getAjscr() {
		return ajscr;
	}
	public void setAjscr(String ajscr) {
		this.ajscr = ajscr;
	}
	public String getAjscrbh() {
		return ajscrbh;
	}
	public void setAjscrbh(String ajscrbh) {
		this.ajscrbh = ajscrbh;
	}
	public String getXzcfjdswh() {
		return xzcfjdswh;
	}
	public void setXzcfjdswh(String xzcfjdswh) {
		this.xzcfjdswh = xzcfjdswh;
	}
	public String getJyaq() {
		return jyaq;
	}
	public void setJyaq(String jyaq) {
		this.jyaq = jyaq;
	}
	public String getClyj() {
		return clyj;
	}
	public void setClyj(String clyj) {
		this.clyj = clyj;
	}
	public String getXzssqk() {
		return xzssqk;
	}
	public void setXzssqk(String xzssqk) {
		this.xzssqk = xzssqk;
	}
	public String getCfzxqk() {
		return cfzxqk;
	}
	public void setCfzxqk(String cfzxqk) {
		this.cfzxqk = cfzxqk;
	}
	public String getRemark() {
		return remark;
	}
	public void setRemark(String remark) {
		this.remark = remark;
	}
	public String getAreaId() {
		return areaId;
	}
	public void setAreaId(String areaId) {
		this.areaId = areaId;
	}
	
}
