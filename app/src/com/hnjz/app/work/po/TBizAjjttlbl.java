package com.hnjz.app.work.po;

import java.util.Date;
import com.hnjz.sys.po.BaseObject;

/**
* T_BIZ_AJJTTLBL 实体类
 * 作者：zhangqingfeng
 * 生成日期：Mar 22 2016
 * 功能描述：案件集体讨论笔录
 */ 

@SuppressWarnings("serial")
public class TBizAjjttlbl extends BaseObject {
	/** 任务ID */
	private String taskid;
	/** 案件名称 */
	private String ajmc;
	/** 案件讨论时间（起） */
	private Date tlstartsj;
	/** 案件讨论时间（止） */
	private Date tlendsj;
	/** 讨论地点 */
	private String tldd;
	/** 主持人 */
	private String zcr;
	/** 主持人职务 */
	private String zcrzw;
	/** 记录人 */
	private String jlr;
	/** 记录人职务 */
	private String jlrzw;
	/** 参加人员 */
	private String cjrys;
	/** 人员 */
	private String ry;
	/** 承办人汇报案件情况 */
	private String ajqk;
	/** 陈述（听证）情况 */
	private String csqk;
	/** 参加讨论人员意见 */
	private String tlyj;
	/** 结论意见 */
	private String tljl;
	/** 数据区域过滤标识 */
	private String areaId;
	public String getTaskid() {
		return taskid;
	}
	public void setTaskid(String taskid) {
		this.taskid = taskid;
	}
	public String getAjmc() {
		return ajmc;
	}
	public void setAjmc(String ajmc) {
		this.ajmc = ajmc;
	}
	public Date getTlstartsj() {
		return tlstartsj;
	}
	public void setTlstartsj(Date tlstartsj) {
		this.tlstartsj = tlstartsj;
	}
	public Date getTlendsj() {
		return tlendsj;
	}
	public void setTlendsj(Date tlendsj) {
		this.tlendsj = tlendsj;
	}
	public String getTldd() {
		return tldd;
	}
	public void setTldd(String tldd) {
		this.tldd = tldd;
	}
	public String getZcr() {
		return zcr;
	}
	public void setZcr(String zcr) {
		this.zcr = zcr;
	}
	public String getZcrzw() {
		return zcrzw;
	}
	public void setZcrzw(String zcrzw) {
		this.zcrzw = zcrzw;
	}
	public String getJlr() {
		return jlr;
	}
	public void setJlr(String jlr) {
		this.jlr = jlr;
	}
	public String getJlrzw() {
		return jlrzw;
	}
	public void setJlrzw(String jlrzw) {
		this.jlrzw = jlrzw;
	}
	public String getCjrys() {
		return cjrys;
	}
	public void setCjrys(String cjrys) {
		this.cjrys = cjrys;
	}
	public String getRy() {
		return ry;
	}
	public void setRy(String ry) {
		this.ry = ry;
	}
	public String getAjqk() {
		return ajqk;
	}
	public void setAjqk(String ajqk) {
		this.ajqk = ajqk;
	}
	public String getCsqk() {
		return csqk;
	}
	public void setCsqk(String csqk) {
		this.csqk = csqk;
	}
	public String getTlyj() {
		return tlyj;
	}
	public void setTlyj(String tlyj) {
		this.tlyj = tlyj;
	}
	public String getTljl() {
		return tljl;
	}
	public void setTljl(String tljl) {
		this.tljl = tljl;
	}
	public String getAreaId() {
		return areaId;
	}
	public void setAreaId(String areaId) {
		this.areaId = areaId;
	}

}

