...
<html>
	...
	<body>
        ...
        <div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;">
			<form id="queryForm" action="queryLawobjList.json" method="post">
...
////////////////////////////////////////////////////////////////////////////////////
function initH(){
	var hh=$(window).height() - $("#divTitle").outerHeight()-13;
	$("#rbblist").height(hh);
}
initH();
//数据表格自使用宽度
...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		fitColumns:true,
		nowrap: false,
		url:'queryLawobjList.json?orgId='+$('#ssjgbm').val(),
		...
		columns:[[
			...
			{field:'operate',title:'操作', align:"center", halign:'center',width:30,
				formatter:function(value,rowData,rowIndex){  
		    		return '<a class="b-link" onclick="info(\''+ rowData.id +','+rowData.lawobjType+'\')">查看</a>';  
		       }
				}
		]],
...
function info(obj){
	...
	var width=screen.width * 0.8;
  	...
  	parent.layerIframe(2,'lawobjInfo.htm?id='+arr[0],title,width,height);
}
...
$('#J-form-export').click(function(){
	...
	$('#download').attr('src','exportLawobjList.json?name='+encodeURIComponent(name)+'&lawobjType='+lawobjType+'&regionId='+regionId+'&regionName='+encodeURIComponent(regionName)+'&orgId='+orgId+'&orgName='+encodeURIComponent(orgName));
});
</script>