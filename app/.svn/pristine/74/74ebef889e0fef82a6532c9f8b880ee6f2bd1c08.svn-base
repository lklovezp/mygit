...
<html>
	...
	<body>
        ...
        <div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;">
			<form id="queryForm" action="queryXqyzList.json?isActive=Y" method="post">
...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		fitColumns:true,
		nowrap: false,
		url:'queryXqyzList.json?isActive=Y&qyzt=0&orgId='+$('#ssjgbm').val(),
		...
		onDblClickRow: function(rowIndex, rowData){
			//All.ShowModalWin('xqyzInfo.htm?id='+rowData.id, '查看畜禽养殖信息');
			var width=screen.width * 0.8;
		  	var height=screen.height * 0.8-50;
		  	var title='查看畜禽养殖信息';
		  	parent.layerIframe(2,'xqyzInfo.htm?id='+rowData.id,title,width,height);
		}
	});
	initPager();
});
...
function transfer(obj){
	//All.ShowModalWin('lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=07&multi=false&lawobjId='+$(obj).attr("id"), '创建人转移',300, 380);
	parent.layer.open({
        type: 2,
        title: '创建人转移',
        area: ['300px', '420px'],
        content: 'lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=07&multi=false&lawobjId='+$(obj).attr("id"),
        end: function () {
            refresh();
        }
...
function edit(obj){
	...
	parent.layer.open({
            ...
            content: 'xqyzEdit.htm?id='+$(obj).attr("id"), 
            end: function () {
            	refresh();
            }
...
function info(obj){
	//All.ShowModalWin('xqyzInfo.htm?id='+$(obj).attr("id"), '查看畜禽养殖信息');
	var width=screen.width * 0.8;
  	var height=screen.height * 0.8-50;
  	var title='查看畜禽养殖信息';
  	parent.layerIframe(2,'xqyzInfo.htm?id='+$(obj).attr("id"),title,width,height);
}
