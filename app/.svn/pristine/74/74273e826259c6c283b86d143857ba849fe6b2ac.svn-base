...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>锅炉信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" id="id" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="04"/>
			<div id="condition">
				${innerHtml }
			</div>
...
$(document).ready(function(){
	...
	$("#gl_wghzrr").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	...
	$("#J-from-reset").click(function(){
		   ...
		   var $selectedvalue = $("input[id='gl_qysczt']:checked").val();
		   if ($selectedvalue == 'Y') {
				$("#season").hide();
				$("#gl_qysczt").val("Y");
		   }
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存锅炉信息:", data.msg, "info", function () {
							    //self.close();
							...
							$.messager.alert('保存锅炉信息:',data.msg);
						}
					}
				});
			}
  ...
  //附件列表
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前锅炉信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前锅炉信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
...
function setUserValue(id,name){
	//$("#gl_wghzrr").linkbox("setValue", {id:id,name:name});
	$("#gl_wghzrr").val(name);
	$("#gl_wghzrrid").val(id);
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='gl_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='gl_qysczt']").each(function(){
