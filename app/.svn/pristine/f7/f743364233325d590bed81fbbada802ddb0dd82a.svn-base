...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		pagination:true,
		fitColumns:true,
		url:'queryGlxxList.json?isActive=Y&qyzt=0&orgId='+$('#ssjgbm').val(),
		onLoadSuccess:function(data){
			$(this).datagrid('doCellTip',{'max-width':'300px','delay':500});
		},
		columns:[[
			{field:'id',hidden:true},
			{field:'name',title:'锅炉名称', align:"left", halign:'center',width:100},
		...
		onDblClickRow: function(rowIndex, rowData){
			//All.ShowModalWin('glxxInfo.htm?id='+rowData.id, '查看锅炉信息');
			var width=screen.width * 0.8;
		  	var height=screen.height * 0.8-50;
		  	var title='查看锅炉信息';
		  	parent.layerIframe(2,'glxxInfo.htm?id='+rowData.id,title,width,height);
		}
	});
	initPager();
});
...
function edit(obj){
	...
	parent.layer.open({
        ...
        title: '编辑锅炉信息',
        area: ['1100px', ($(window).height()+120)+'px'],
        content: 'glxxEdit.htm?id='+$(obj).attr("id"), 
        end: function () {
            //location.reload();
            refresh();
        }
...
$("#daoru").click(function(){
	//All.ShowModalWin('importPage.htm?lawObjType=04', '工业污染源导入', 600, 350);
	//refresh();
	layer.open({
        type: 2,
        title: '锅炉导入',
        area: ['600px', ($(window).height()-50)+'px'],
        content: 'importPage.htm?lawObjType=04', 
        end: function () {
            //location.reload();
        	refresh();
        }
...
function info(obj){
	//All.ShowModalWin('glxxInfo.htm?id='+$(obj).attr("id"), '查看锅炉信息');
	var width=screen.width * 0.8;
  	var height=screen.height * 0.8-50;
  	var title='查看锅炉信息';
  	parent.layerIframe(2,'glxxInfo.htm?id='+$(obj).attr("id"),title,width,height);
}

//创建人转移
function transfer(obj){
	//All.ShowModalWin('lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=04&multi=false&lawobjId='+$(obj).attr("id"), '创建人转移',300, 380);
	parent.layer.open({
        type: 2,
        title: '创建人转移',
        area: ['300px', '420px'],
        content: 'lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=04&multi=false&lawobjId='+$(obj).attr("id"),
        end: function () {
            //location.reload();
            refresh();
        }
