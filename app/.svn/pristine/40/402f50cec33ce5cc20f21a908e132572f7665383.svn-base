...
$("#xinjian").click(function(){
	//All.ShowModalWin('ysyEdit.htm', '新建饮食业信息', 1200, 1200);
	//refresh();
	parent.layer.open({
        type: 2,
        title: '新建饮食业信息',
...
/*
function del(obj){
	$.messager.confirm('饮食业信息管理', '确定要删除吗？', function(r){
		if (r){
			$.ajax({
...
$("#daoru").click(function(){
	//All.ShowModalWin('importPage.htm?lawObjType=09', '工业污染源导入', 600, 350);
	//refresh();
	layer.open({
        type: 2,
        title: '饮食业导入',
        area: ['600px', ($(window).height()-50)+'px'],
        content: 'importPage.htm?lawObjType=09', 
        end: function () {
            //location.reload();
            refresh();
        }
