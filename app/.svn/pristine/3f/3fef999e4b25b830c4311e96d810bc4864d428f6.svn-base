...
.formtable, .formtable th, .formtable td{border-color:#dddddd;}
	...
	<body>
		<div class="h1_1" id="divTitle">建筑工地</div>
...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		fitColumns:true,
		nowrap:false,
		url:'queryJzgdList.json?isActive=Y&regionid=${regionid}',
		onLoadSuccess:function(data){
			$(this).datagrid('doCellTip',{'max-width':'300px','delay':500});
		},
		columns:[[
			{field:'id',hidden:true},
			{field:'name',title:'建筑工地名称', align:"left", halign:'center',width:100},
...
function info(id){
	...
	var title='查看建筑工地信息';
	parent.layerIframe(2,'scxxInfo.htm?id='+id,title,width,height);
}
</script>