...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>服务业信息</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" name="id" value="${lawobj.id }"/>
			<input type="hidden" name="lawobjtype" value="08"/>
...
$(document).ready(function(){
	...
	$("#fwy_wghzrr").colorbox({iframe:true,width:"300", height:"380",href:"taskUserPubQuery.htm?all=true&notShowZj=false&methodname=setUserValue&multi=true"});
	$("#fjlx").combobox({
	...
	$("#fwy_dwmc").val('${jsxmForm.dwmc }');
	$("#fwy_dz").val('${jsxmForm.address }');
	$("#fwy_fddbr").val('${jsxmForm.fddbr }');
	$("#fwy_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
	$("#fwy_hbfzr").val('${jsxmForm.hbfzr }');
	$("#fwy_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
	$("#fwy_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	
	$("#J-from-reset").click(function(){
		   $("#queryForm").form("reset");
		   var $selectedvalue = $("input[id='fwy_qysczt']:checked").val();
		   if ($selectedvalue == 'Y') {
				$("#season").hide();
				$("#fwy_qysczt").val("Y");
		   }
			...
			if($("#queryForm").form("validate")){
				$(form).ajaxSubmit({
					...
					success: function(data){
						if(data.state){
							/*$.messager.alert("保存服务业信息:", data.msg, "info", function () {
							    //self.close();
							...
							$.messager.alert('保存服务业信息:',data.msg);
						}
					}
				});
			}
    ...
    $("#fileupload").click(function(){
		...
		if(id!=null && id!=''){
			...
			if($("#queryForm").form("validate")){
				/*$.messager.confirm('操作确认', '上传附件需要保存当前服务业信息，是否继续？', function(r){
					if(r){
				...
				/*var index=layer.confirm('上传附件需要保存当前服务业信息，是否继续？', {
			     	icon: 3, 
			        title:'操作确认'
			     }, function(index){
			    	 $("#queryForm").ajaxSubmit({
...
function setUserValue(id,name){
	//$("#fwy_wghzrr").linkbox("setValue", {id:id,name:name});
	$("#fwy_wghzrr").val(name);
	$("#fwy_wghzrrid").val(id);
	jQuery().colorbox.close();
}
...
$(document).ready(function() {
	...
	$("input[id='fwy_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='fwy_qysczt']").each(function(){
