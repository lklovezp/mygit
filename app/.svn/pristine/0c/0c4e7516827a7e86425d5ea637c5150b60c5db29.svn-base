...
<body>
	...
	<div class="block">
		<p class="subTit">二、水环境保护目标分布（下游5公里范围内）</p>
		<div class="t-r" id="extraBtn" style="margin:5px 30px;">
			<a class="b-link" onclick="addWaterProject()">添加</a>
		</div>
		<!--数据表格-->
	...
	<script type="text/javascript">
		...
		$('#content1').datagrid({
			...
			url : 'waterList.json?isActive=Y&pid='+parent.pid,//请求数据的超链接地址
			//可以设置每页记录条数的列表  
			...
			{field : 'qxlxdm',	title : '废水主要排水去向类型代码 ',sortable : true,align : 'center',width : 2},
			{field : 'stmc', title : '废水受纳水体名称',align : 'center',width : 2},
			{field : 'qjstmc', title : '清净下水受纳水体名称 ',align : 'center',width : 2},
		...
		$('#content2').datagrid({
			...
			pageList: [5,10,15,20],//可以设置每页记录条数的列表  
			remoteSort : false,//是否远程排序
			url : 'waterProjectList.json?isActive=Y&pid='+parent.pid,//请求数据的超链接地址
			//可以设置每页记录条数的列表  
			onLoadSuccess : function(data) {
				//datagrid鼠标经过提示单元格内容(需要在easyui.js中写扩展的doCellTip方法)
				$(this).datagrid('doCellTip', {'max-width' : '300px','delay' : 500});
			...
			{field : 'sl', title : '规模数量',align : 'center',width : 2},
			{field : 'operate',title : '操作',align : 'center',width : 1} ] ],
			onDblClickRow : function(rowIndex, rowData) {//双击某一行，查看详情
				info();
			},
		...
		//查看副产品详情
		function infoWaterProject(obj) {
			$.fn.colorbox({
				iframe : true,
				width : "823",
				height : "480",
				href : "infoWaterProject.htm?id="+obj.id
			});
		}
		...
		function add() {
			$.fn.colorbox({
				...
				width : "823",
				height : "480",
				href : "waterAdd.htm?pid="+parent.pid
			});
		}
		//增加
		function addWaterProject() {
			$.fn.colorbox({
				iframe : true,
				width : "823",
				height : "480",
				href : "waterProjectAdd.htm?pid="+parent.pid
			});
		}
		
		//数据表格每行的“修改”
		function modifyWater(obj) {
			$.fn.colorbox({
				iframe : true,
				width : "823",
				height : "480",
				href : "editWater.htm?id="+obj.id
			});
		}
		//数据表格每行的“修改”
		function modifyWaterProject(obj) {
			$.fn.colorbox({
				iframe : true,
				width : "823",
				height : "480",
				href : "editWaterProject.htm?id="+obj.id
			});
		}
		...
		/*删除主产品
		function delWater(target) {
			
			$.messager.confirm('水环境基本状况', '确定要删除吗?', function(r) {
				if (r){
					$.ajax({
						url: "delWater.json?id="+target.id,
		...
		/*删除副产品
        function delWaterProject(target) {
			    $.messager.confirm('水环境保护目标分布（下游5公里范围内）', '确定要删除吗?', function(r) {
				if (r){
					$.ajax({
						url: "delWaterProject.json?id="+target.id,
						success:function(data){
							alert(data.msg);
