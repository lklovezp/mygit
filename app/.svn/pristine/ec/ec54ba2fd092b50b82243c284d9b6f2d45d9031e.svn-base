...
<div id ="state" class="lawObjectWrap">
  <div class="state">
 	...
 	<div class="taskStateC">
  	<p><a class="kaBtn_blue solid" onclick="getdpfrw()">待派发（${total}）</a> <span class="">待派发（${total}）</span><span>已修改完成时间（${total}）</span></p>
    <p><a class="kaBtn_green solid" onclick="getdbrw()">待办理（${total}）</a> <span class="">待办理（${total}）</span><span class=" ">逾期未办理（${total}）</span><span class=" ">待审核（${total}）</span></p>
    <p><a class="kaBtn_orange solid" onclick="getyblrw()">已办理（${total}）</a> <span class="">已办理（${total}）</span><span class=" ">已上报（${total}）</span><span class=" ">已归档（${total}）</span></p></div>
  </div>
</div>
...
function createTask() {
    if(rwState == "" || rwState == null){
    	if(rowid == null || rowid == ""){
			window.location.href="pfPage.htm?lx="+"zfjc";
		}else{
			window.location.href="pfPage.htm?applyId="+rowid+"&lx="+"zfjc";
		}
    }else{
    	window.location.href="pfPage.htm?lx="+"zfjc";
    }
