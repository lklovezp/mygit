   ...
   <div style="padding:25px;">
    ...
    <div class="rightCon">
    	<div class="searchWrap" style=" padding:1px 25px 0px;">
            ...
            <form id="queryForm" action="userQuery.json" method="post">
...
function initH(){
	var hh=$(window).height() - $("#searchDiv").outerHeight()-$("#czdiv").outerHeight()-90;
	$("#dataDiv").height(hh);
}
initH();
...
$('#data').datagrid({
	...
	fitColumns : true,
	pagination:true,
	url:'userQuery.json?areaid=${areaid}',//请求数据的超链接地址
	...
	columns:[[//设置表头
	    ...
	    {field:'name',title:'用户名称',align:'center',width:3},
	    {field:'userName',title:'登录名',width:3},
		{field:'workmobile',title:'工作手机号',align:'center',width:3},
		{field:'personmobile',title:'个人手机号',align:'center',width:3},
		{field:'lawnumber',title:'执法证号',width:3},
		{field:'org',title:'所属部门',width:3}
	]]
    ...
    if (rows.length > 1) {
    	...
    	var orgid=$("#orgid").val();
    	 parent.layer.open({
    	        ...
    	        title:'修改用户',
    	        area: ['1100px','650px'],
    	        content:'editUser.htm?orgid='+orgid+'&id='+id, 
    	        end: function () {
    	            //location.reload();
    	            refresh();
    	        }
...
$("#del").click(function(){
	var rows = $('#data').datagrid('getSelections');
	 var  ids = [];
	  ...
	  if(null !=ids && ""!=ids){
		var index=layer.confirm('确定要删除用户吗？', {
	     	icon: 3, 
	        title:'删除用户'
	     }, function(index){
	    	 $.ajax( {
					url : "delUser.json?ids="+ids,
					success : function(data) {
...
$("#xinjian").click(function(){
	...
	if(null != orgid && "" != orgid){
	 parent.layer.open({
        ...
        title:'新建用户',
        area: ['1100px','650px'],
        content:'editUser.htm?orgid='+orgid, 
        end: function () {
            //location.reload();
            refresh();
        }
