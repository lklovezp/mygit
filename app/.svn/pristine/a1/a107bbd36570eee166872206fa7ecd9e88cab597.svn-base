...
<div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;top:30px;">
   <form id="queryForm" method="post" action="getXftsrwList.json">
   ...
   <table class="queryTable" width="98%" border="0" cellpadding="0" cellspacing="0" style="margin:0 auto;">
         <tr>
           ...
           <th width="10%"><a href="#" class="task_zflx" id="selectXfdjb">信访登记表</a></th>
			<td width="15%">
				<input type="hidden" id="xfdjbId" name="xfdjbId" value="${work.xfdjbId}"/>
				<input class="y-text" type="text" id="xfbh" readonly="readonly" value="${xfbh}" />
			</td>
...
<h1 id="title" class="h1_1 topMask" style="padding-top: 10px;">信访投诉（<font color="red">${total}</font>条）</h1>
...
$(document).ready(function(){
	...
	$("#selectXfdjb").click(function(){
		var href="choseeXfdjb.htm?type=edit";
		var width=1000;
	  	var height=420;
    ...
    $('#data').datagrid({
	            ...
	            columns:[[
					{field:'dbworkName1',title:'任务名称',width:200},
					{field:'workNote1',title:'任务内容',halign:'center',align:'left',width:200},
					{field:'rwly',title:'任务来源',align:'center',width:100},
					{field:'jjcd',title:'紧急程度',align:'center',width:120},
					{field:'yqwcsx',title:'要求完成时限',align:'center',width:100},
					{field:'createby',title:'派发人',align:'center',width:60},
					{field:'zxrName',title:'主办人',align:'center',width:60},
...
function actionOper(code,action,actionType,applyId,taskId,text){
	if(actionType=='AJAX'){
		   ...
		   parent.$.colorbox({
			...
			//改为弹出对话框窗口
			var href = '${ctx}/'+action+'&applyId='+applyId+'&taskId='+taskId+'&lx=2'; 
		  ...
		  //	parent.layerIframe(2,href,"待办任务",width,height);
			...
			// window.location.href='${ctx}/'+action+'&applyId='+applyId+'&taskId='+taskId+'&lx=2';
