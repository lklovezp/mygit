...
<html>
	...
	<body style="padding-bottom: 70px;">
		<div class="h1_1" id="divTitle"><c:if test="${lawobj.id == null }">新建</c:if><c:if test="${lawobj.id != null }">编辑</c:if>工业污染源</div>
		<form id="queryForm" action="" method="post">
			<input type="hidden" id="lawobjId" name="id" value="${lawobj.id}"/>
			<input type="hidden" id="lawobjType" name="lawobjtype" value="01"/>
...
$(document).ready(function(){
	...
	$("#gywry_dwmc").val('${jsxmForm.dwmc }');
	$("#gywry_dz").val('${jsxmForm.address }');
	$("#gywry_fddbr").val('${jsxmForm.fddbr }');
	$("#gywry_fddbrlxdh").val('${jsxmForm.fddbrlxdh }');
	$("#gywry_hbfzr").val('${jsxmForm.hbfzr }');
	$("#gywry_hbfzrlxdh").val('${jsxmForm.hbfzrlxdh }');
	$("#gywry_ssxzq").combotree('setValue','${jsxmForm.regionid }');
	</c:if>
	...
	$("#J-from-reset").click(function(){
		   ...
		   var $selectedvalue = $("input[id='gywry_qysczt']:checked").val();
		   if ($selectedvalue == 'Y') {
				 $("#season").hide();
				 $("#gywry_qysczt").val("Y");
		   }
...
$(document).ready(function() {
	...
	$("input[id='gywry_qysczt']").blur(function(){
		var yue="";
		$("#season input[id='gywry_qysczt']").each(function(){
