...
<h1 id="title" class="h1_1 topMask" style="padding-top: 10px;">待办任务（<font color="red">${total}</font>条）</h1>
    ...
    if(xfdjId == "" || xfdjId == null){
	    ...
	    $('#data').datagrid({
            ...
            columns:[[
				...
				{field:'rwlx1',title:'任务类型',halign:'center',align:'left',width:100,sortable:true},
...
function actionOper(code,action,actionType,applyId,taskId,text){
	if(actionType=='AJAX'){
		   ...
		   parent.$.colorbox({
			...
			//改为弹出对话框窗口
			var href = '${ctx}/'+action+'&applyId='+applyId+'&taskId='+taskId+'&lx=1'; 
