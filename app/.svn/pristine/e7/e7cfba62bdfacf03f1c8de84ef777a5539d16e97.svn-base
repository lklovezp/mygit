...
<body>
        ...
        <div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;">
			<form id="queryForm" action="queryZzyList.json" method="post">
...
$(document).ready(function(){
	...
	$('#data').datagrid({
		...
		nowrap:false,
		fitColumns:true,
		url:'queryZzyList.json?isActive=Y&qyzt=0&orgId='+$('#ssjgbm').val(),
		...
		onDblClickRow: function(rowIndex, rowData){
			//All.ShowModalWin('zzyInfo.htm?id='+rowData.id, '查看制造业信息');
			var width=screen.width * 0.8;
		  	var height=screen.height * 0.8-50;
		  	var title='查看制造业信息';
		  	parent.layerIframe(2,'zzyInfo.htm?id='+rowData.id,title,width,height);
		}
	});
	initPager();
});
...
$("#xinjian").click(function(){
	//All.ShowModalWin('zzyEdit.htm', '新建制造业信息', 1000, 800);
	//refresh();
	parent.layer.open({
        type: 2,
        title: '新建制造业信息',
...
function edit(obj){
	//All.ShowModalWin('zzyEdit.htm?id='+$(obj).attr("id"), '编辑制造业信息', 1000, 800);
	//refresh();
	parent.layer.open({
        type: 2,
        title: '编辑制造业信息',
        area: ['1100px', ($(window).height()+120)+'px'],
        content: 'zzyEdit.htm?id='+$(obj).attr("id"), 
        end: function () {
        	refresh();
        }
...
/*
function del(obj){
	$.messager.confirm('制造业信息管理', '确定要删除吗？', function(r){
		if (r){
			$.ajax({
...
$("#daoru").click(function(){
	//All.ShowModalWin('importPage.htm?lawObjType=10', '工业污染源导入', 600, 350);
	//refresh();
	layer.open({
        type: 2,
        title: '制造业导入',
        area: ['600px', ($(window).height()-50)+'px'],
        content: 'importPage.htm?lawObjType=10', 
        end: function () {
           refresh();
        }
...
function info(obj){
	//All.ShowModalWin('zzyInfo.htm?id='+$(obj).attr("id"), '查看制造业信息');
	var width=screen.width * 0.8;
  	var height=screen.height * 0.8-50;
  	var title='查看制造业信息';
  	parent.layerIframe(2,'zzyInfo.htm?id='+$(obj).attr("id"),title,width,height);
}

//创建人转移
function transfer(obj){
	//All.ShowModalWin('lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=10&multi=false&lawobjId='+$(obj).attr("id"), '创建人转移',300, 380);
	parent.layer.open({
        type: 2,
        title: '创建人转移',
        area: ['300px', '420px'],
        content: 'lawobjUserPubQuery.htm?all=true&notShowZj=false&lawobjtypeid=10&multi=false&lawobjId='+$(obj).attr("id"),
        end: function () {
           refresh();
        }
