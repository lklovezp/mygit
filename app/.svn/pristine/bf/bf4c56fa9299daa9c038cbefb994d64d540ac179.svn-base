...
<body>
	<div class="block">
		<p class="subTit">企业环境应急处置及救援资源</p>
		<div class="t-r" id="extraBtn" style="margin:5px 30px;">
			<a class="b-link" onclick="add()">添加</a>
		</div>
		<!--数据表格-->
		<div class="divContainer" id="zycpList" style="height:460px;">
			<table id="data1" fit="true"></table>
		</div>
	</div>
...
<script type="text/javascript">
		...
		$('#data1').datagrid({
			...
			url : 'yjczjjyzyList.json?isActive=Y&pid='+pid,//请求数据的超链接地址
			//可以设置每页记录条数的列表  
			...
			{field : 'type',	title : '应急处置设施（备）',sortable : true,align : 'center',width : 2},
			{field : 'wzmc', title : '物资名称',align : 'center',width : 2},
		...
		function info(obj) {
			$.fn.colorbox({
				...
				width : "823",
				height : "480",
				href : "yjczjjyzyInfo.htm?id="+obj.id
			});
		}
		...
		function add() {
			$.fn.colorbox({
				...
				width : "823",
				height : "480",
				href : "yjczjjyzyAdd.htm?pid="+parent.pid
			});
		}
		...
		function modify(obj) {
			$.fn.colorbox({
				...
				width : "823",
				height : "480",
				href : "editYjczjjyzy.htm?id="+obj.id
			});
		}
		...
		function del(target) {
			...
			$.messager.confirm('企业环境应急处置及救援资源', '确定要删除吗?', function(r) {
				if (r){
					$.ajax({
						url: "delYjczjjyzy.json?id="+target.id,
						success:function(data){
							alert(data.msg);
							var p = $('#data1').datagrid('getPager');
