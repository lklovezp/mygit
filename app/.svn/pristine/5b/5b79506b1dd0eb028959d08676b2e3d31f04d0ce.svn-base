...
<script type="text/javascript">
		...
		function initData(){
			$('#data').datagrid({
				...
				remoteSort : false,//是否远程排序
				url : 'queryJbxxList.json?lawobjId='+parent.pid,//请求数据的超链接地址
				//可以设置每页记录条数的列表  
				...
				{field : 'dwmc',	title : '单位名称',sortable : true,align : 'center',width:1},
				{field : 'code', title : '单位代码',align : 'center',width:1},
				{field : 'dwszd', title : '单位地址',align : 'center',width:2},
		...
		initData();
		//查看详情
		function infoJbxx(obj) {
			$.fn.colorbox({
				iframe : true,
				width : "823",
				height : "480",
				href : "infoJbxx.htm?id="+obj.id+"&lawobjId="+parent.pid
			});
		}
		...
		function add() {
			$.fn.colorbox({
				...
				width : "900",
				height : "500",
				href : "qyjbqkAdd.htm?id="+parent.pid
			});
		}
		
		//数据表格每行的“修改”
		function modifyJbxx(obj) {
			$.fn.colorbox({
				iframe : true,
				width : "900",
				height : "500",
				href : "editQyjbqk.htm?id="+obj.id+"&lawobjId="+parent.pid
			});
		}
		...
		//删除主产品
		function delJbxx(target) {
			
			$.messager.confirm('企业环境基本信息', '确定要删除吗?', function(r) {
				if (r){
					$.ajax({
						url: "delJbxx.json?id="+target.id,
