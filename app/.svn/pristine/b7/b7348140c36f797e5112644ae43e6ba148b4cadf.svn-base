...
<body>
<div class="breadClumbs"> 任务&nbsp;&gt;&nbsp;日常办公&nbsp;&gt;&nbsp;${title}</div>
...
<div id="searchForm" class="searchForm" style="padding-top:10px;padding-bottom:21px;top:30px">
   <form id="queryForm" method="post" action="getRcbgrwList.json">
   ...
   <table class="queryTable" width="98%" border="0" cellpadding="0" cellspacing="0" style="margin:0 auto;">
         <tr>
          ...
          <th width="10%">派发人	</th>
		  <td width="25%"><input type="hidden" id="pfrId" name="pfrId"/>
...
<h1 id="title" class="h1_1 topMask" style="padding-top: 10px;">日常办公</h1>
...
$(document).ready(function(){
    ...
    $('#data').datagrid({
	            ...
	            columns:[[
					...
					{field:'yqwcsx',title:'要求完成时限',align:'center',width:125},
...
function actionOper(code,action,actionType,applyId,taskId,text){
	if(actionType=='AJAX'){
		   ...
		   parent.$.colorbox({
			...
			//改为弹出对话框窗口
			var href = '${ctx}/'+action+'&applyId='+applyId+'&taskId='+taskId+'&lx=4'; 
		  	var width=screen.width * 0.9;
		  	var height=screen.height * 0.8-50;
		  //	parent.layerIframe(2,href,"待办任务",width,height);
			...
			// window.location.href='${ctx}/'+action+'&applyId='+applyId+'&taskId='+taskId+'&lx=4';
