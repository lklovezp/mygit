...
function review(obj) {
    ...
    arr = value.split(",");
    if (arr[1] <= 4194304) {
        All.ShowModalWin('preview.htm?id=' + arr[0], '预览', 1100, 900);
    } else {
        alert("文件太大，请下载预览。");
    }
}
function reviewImg(obj) {
    var value = obj.id;
    var arr = new Array();
    arr = value.split(",");
    parent.layer.open({
        type: 2,
        title: '附件查看',
        area: ['90%', ($(parent.window).height()-35)+'px'],
        content: 'imgView.htm?id=' + arr[0]
    });
}

function pdfreview(obj) {
    var value = obj.id;
    var arr = new Array();
    arr = value.split(",");
    parent.layer.open({
        ...
        title: '附件查看',
        area: ['90%', ($(parent.window).height()-35)+'px'],
        content: 'pdfView.htm?id=' + arr[0]
    });
}

function docreview(obj) {
    var value = obj.id;
    var arr = new Array();
    arr = value.split(",");
    if (arr[1] <= 4194304) {
        parent.layer.open({
            ...
            title: '附件查看',
            area: ['90%', ($(parent.window).height()-35)+'px'],
            content: 'docpreview.htm?id=' + arr[0]
        });
    } else {
        alert("文件太大，请下载预览。");
    }
